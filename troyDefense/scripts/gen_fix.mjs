...
const manifest = JSON.parse(readFileSync(path.join(ROOT, "scripts/asset_manifest_fix.json"), "utf-8"));
...
function buildPrompt(item) {
  ...
  const chroma = "**背景規則（最高優先，違反即作廢）**：整張圖的背景必須是 100% 純洋紅色 #FF00FF 的單一色塊。" +
    "**畫面中除了角色本身以外，不可出現任何東西**——不可有地面、草地、石板、泥土、場景、" +
    "environment、環境光暈、地板陰影、光影投射、裝飾物、外框或文字。" +
    "角色要像被去背後貼在純色底上一樣乾淨（角色的落地陰影由遊戲引擎另外繪製，這裡不要畫）。" +
    "主體置中並盡量填滿畫面，主體本身絕對不可出現任何洋紅或粉紅色。";
