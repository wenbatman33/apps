export function createGeneratedAssets(scene) {
    createShip(scene);
    createEnemies(scene);
    createBullets(scene);
    createPickups(scene);
...
function createShip(scene) {
    if (scene.textures.exists('player-ship'))
        return;
...
function createEnemyTexture(scene, key, width, height, primary, secondary) {
    const g = scene.add.graphics();
...
function createPickups(scene) {
    if (scene.textures.exists('pickup-power') && scene.textures.exists('pickup-bomb'))
        return;
    const g = scene.add.graphics();
...
function createExplosion(scene) {
    const hasAllFrames = Array.from({ length: 8 }, (_, index) => scene.textures.exists(`explosion-${index}`)).every(Boolean);
    if (!hasAllFrames) {
        for (let index = 0; index < 8; index += 1) {
            if (scene.textures.exists(`explosion-${index}`))
                continue;
            const g = scene.add.graphics();
...
function createBackgroundTiles(scene) {
    if (scene.textures.exists('bg-dawn') && scene.textures.exists('bg-orbit'))
        return;
    if (scene.textures.exists('bg-dawn'))
        return;
    const g = scene.add.graphics();
    g.fillGradientStyle(0x102552, 0x1d5582, 0xffb06d, 0x3a1d4f, 1);
    g.fillRect(0, 0, 432, 936);
    g.lineStyle(1, 0xffffff, 0.08);
    for (let y = 0; y < 936; y += 52)
        g.lineBetween(0, y, 432, y + 18);
