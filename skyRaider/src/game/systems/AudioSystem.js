import { MASTER_VOLUME } from '../constants.js';
...
const SHOT_FILES = {
    ...
    weaponSwitch: 'assets/sound/sfx/weapon-switch.mp3',
...
export class AudioSystem {
    ...
    unlock() {
        ...
        Object.keys(SHOT_FILES).forEach((key) => void this.loadBuffer(key));
    }
    // 射擊音效：依武器種類使用不同素材與音高
    shoot(weapon, now, power = 1) {
        ...
        if (!config)
            return;
        if (!this.canPlay(`shoot-${weapon}`, now, config.minGapMs))
            return;
        const rate = config.rate * (1 + (Math.random() * 2 - 1) * config.jitter);
        this.playBuffer(weapon, config.volume, rate);
    }
    // 開火期間維持掃射循環（重複呼叫是安全的）
    startVulcanLoop(power) {
        const context = this.getContext();
    ...
    // 追蹤導彈發射
    missile(now) {
        if (!this.canPlay('missile', now, 220))
            return;
        this.playBuffer('missile', 0.35, 1.18);
    }
    enemyHit(now, isLarge = false) {
        if (isLarge) {
            // 打到 boss / 中 boss：使用專屬金屬撞擊音
            if (!this.canPlay('boss-hit', now, 70))
                return;
        ...
        if (!this.canPlay('enemy-hit', now, 55))
            return;
        this.playBuffer('enemyHit', 0.26, 1 + (Math.random() * 2 - 1) * 0.08);
    }
    // 換武器（與火力強化的 levelup 音效區隔開）
    weaponSwitch() {
        this.playBuffer('weaponSwitch', 0.65);
    }
    // Boss 登場警報
    bossWarning() {
    ...
    explosion(kind) {
        ...
        if (large) {
            const key = Math.random() < 0.5 ? 'bossExplode1' : 'bossExplode2';
            this.playSfx(key);
        }
        else {
            const key = Math.random() < 0.5 ? 'enemyExplode1' : 'enemyExplode2';
            this.playSfx(key);
        }
    }
    pickup(kind) {
    ...
    async loadBuffer(key) {
        if (this.buffers.has(key) || this.loading.has(key))
            return;
        const context = this.getContext();
        if (!context)
            return;
    ...
    playSfx(key, volume = MASTER_VOLUME) {
        try {
    ...
    tone(startFrequency, endFrequency, duration, wave, volume, delay = 0) {
        const context = this.getContext();
        if (!context || !this.master)
            return;
    ...
    noise(duration, volume, lowpassFrequency, delay = 0) {
        const context = this.getContext();
        if (!context || !this.master)
            return;
