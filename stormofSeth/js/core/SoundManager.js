...
class SoundManager {
  ...
  startSpin() {
    ...
    // Bandpass → shapes noise into mechanical rumble
    const bpf       = ctx.createBiquadFilter();
    bpf.type        = 'bandpass';
    bpf.frequency.value = 130;
    bpf.Q.value     = 1.8;

    // LFO → wobble the filter frequency for a spinning feel
  ...
  playReelStop(reelIndex = 0) {
    ...
    const ctx  = this._ctx();
    const now  = ctx.currentTime;
    const base = 200 - reelIndex * 18;   // each reel gets a slightly lower pitch
    ...
    // Short noise transient on top (click layer)
  ...
  playBigWin(type = 'big') {
    ...
    seq.forEach((freq, i) => {
      ...
      // Square wave with vibrato for a bright fanfare tone
      const osc     = ctx.createOscillator();
      const gain    = ctx.createGain();
      const lfo     = ctx.createOscillator();
      const lfoGain = ctx.createGain();
