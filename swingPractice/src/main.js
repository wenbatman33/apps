...
import { buildStadium, animateCrowd } from './stadium.js';
...
function updateCamera(dt){
  ...
  if (mode === 'follow' && b.state !== 'idle'){
    ...
    // 直屏（手機）視野較窄，鏡頭拉高退後一點，才看得到外野
    const portrait = innerHeight > innerWidth;
    tPos.set(CAM.batX, CAM.batY + (portrait ? 1.0 : 0), CAM.batZ - (portrait ? 1.6 : 0));
    tLook.set(0, CAM.batLookY + (portrait ? .25 : 0), CAM.batLookZ);
  }
...
G.step = (dt, render = true) => {
  const g = G.game;
  if (g?.slowmo > 0) dt *= 0.42;                        // 全壘打瞬間慢動作
  t += dt;
