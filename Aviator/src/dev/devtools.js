// DEV 微調工具（按 D 或右下齒輪開啟）
// 即時調整版面數值、切換 PC/Mobile、強制指定回合結果，並可一鍵匯出 JSON
...
export class DevTools extends Container {
  constructor(scene) {
    ...
    this.title = txt('DEV 微調工具（D 關閉）', 12, COLORS.text, '800');
  ...
  build() {
    ...
    mkBtn('🎯 拖曳畫面設定飛機巡航點', () => this.scene.toggleDevDrag(), [COLORS.orangeLight, COLORS.orange]);
    mkBtn('立即讓飛機飛走', () => this.scene.forceCrash());
    mkBtn('下一回合鎖 1.00x（秒崩）', () => this.scene.forceNext(1.0));
    mkBtn('下一回合鎖 2.00x', () => this.scene.forceNext(2));
    mkBtn('下一回合鎖 10.00x', () => this.scene.forceNext(10));
    mkBtn('下一回合鎖 100.00x', () => this.scene.forceNext(100));
    mkBtn(`切換版型（目前 ${this.scene.mobile ? 'MOBILE' : 'PC'}）`, () => this.scene.toggleLayoutMode());
    mkBtn('重設此版型數值', () => { this.scene.resetLayout(); this.build(); });
    mkBtn('💾 匯出 JSON（複製到剪貼簿）', () => this.exportJson(), [COLORS.greenLight, COLORS.greenDark]);
  ...
  exportJson() {
    ...
    navigator.clipboard?.writeText(json).then(
      () => { this.status.text = `已複製 ${name} 到剪貼簿，貼回 src/config.js 即可鎖定。`; },
      () => { this.status.text = '已輸出到 console（剪貼簿權限被拒）。'; },
    );
  }
